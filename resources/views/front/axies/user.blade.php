...
<div id="user">
	...
	@if(route_is('home.purchases'))
		...
		<div class="items purchases">
			<div class="header extra">
				<div class="title">{{ __('Purchases') }}</div>
			</div>
				...
				<table class="ui fluid celled borderless unstackable table">
					...
					<tbody>
						...
						<tr class="parent">
							...
							<td class="status {{ $transaction->status }}"><div>{{ __(ucfirst($transaction->status)) }}</div></td>
							...
							<td colspan="4" class="p-0">
								<table class="ui fluid basic borderless unstackable table">
									<tbody>
										...
										<tr>
											<td class="name"><a href="{{ $item->url }}"><i class="right angle icon"></i>{{ $item->name }}</a></td>
	...
	@elseif(route_is('home.favorites'))
		...
		<div class="items collection">
			<div class="header extra">
				<div class="title">{{ __('Collection') }}</div>
			</div>

			<div class="ui four cards" v-if="Object.keys(favorites).length">
				<div class="card fluid" v-for="product in favorites">
					<div class="content">
						<a class="cover" :title="product.name" :href="'/item/' + product.id + '/' + product.slug" :style="'background-image: url(/storage/covers/' + product.cover + ')'"></a>
						<div class="details">
							<a class="name" :title="product.name" :href="'/item/' + product.id + '/' + product.slug">@{{ product.name.shorten(35) }}</a>
							<a class="category":title="product.category_name" :href="'/items/category/' + product.category_slug">@{{ product.category_name.shorten(15) }}</a>
							<i class="trash alternate outline icon mx-0" @click="collectionToggleItem($event, product.id)"></i>
						</div>
					</div>
				</div>
			</div>
	...
	@elseif(route_is('home.user_subscriptions') && config('app.subscriptions.enabled'))
		...
		<div class="items subscriptions">
			<div class="header extra">
				<div class="title">{{ __('Subscriptions') }}</div>
			</div>
			...
			<div class="items-list">
				...
				<div class="content">
					...
					<div class="date">{{ $user_subscription->starts_at }}</div>
					<div class="date">
						@if($user_subscription->ends_at)
						{{ $user_subscription->ends_at }}
						@else
						{{ __('Unlimited') }}
						@endif
					</div>
	...
	@elseif(route_is('home.notifications'))
		<div class="items notifications">
			...
			<div class="items">
				...
				<a class="item @if(!$notification->read) unread @endif" data-id="{{ $notification->id }}"
				data-href="{{ route('home.product', ['id' => $notification->product_id, 'slug' => $notification->slug . ($notification->for == 1 ? '?tab=comments' : ($notification->for == 2 ? '?tab=reviews' : ''))]) }}">

					<div class="image" style="background-image: url({{ asset_("storage/".($notification->for == 0 ? 'covers' : 'avatars')."/{$notification->image}") }})"></div>

					<div class="content">
						<div class="text">{!! __($notification->text, ['product_name' => "<strong>{$notification->name}</strong>"]) !!}</div>
						<div class="date">{{ $notification->updated_at }}</div>
					</div>
				</a>
				@endforeach
			</div>
	...
	@elseif(route_is('home.profile'))
		...
		<div class="items profile">
			<div class="header">
				<div class="content">
					<div class="image" style="background-image: url('{{ asset_("storage/avatars/".($user->avatar ?? 'default.webp').'?v='.time()) }}');"></div>

					<button class="ui yellow circular button mx-0" type="button"  onclick="$('#user .profile input[name=\'avatar\']').click()">
						{{ __('Upload') }}
					</button>
				</div>
			...
			<form class="ui large form" action="{{ route('home.profile') }}" enctype="multipart/form-data" method="post">
				@csrf
				...
				@if(config('affiliate.enabled') || config('app.prepaid_credits.enabled'))				
				<div class="field">
					<label>{{ __('Credits sources to checkout with') }}</label>
					<div class="ui floating selection fluid search multiple dropdown credits-sources" data-sources="{{ old('credits_sources', $user->credits_sources ?? '') }}">
						<input type="hidden" name="credits_sources" >
						<div class="text"></div>
						<div class="menu">
							@if(config('affiliate.enabled'))
							<a class="item" data-value="affiliate_credits">{{ __('Affiliate earnings') }}</a>
							@endif

							@if(config('app.prepaid_credits.enabled'))
							<a class="item" data-value="prepaid_credits">{{ __('Prepaid credits') }}</a>
							@endif
						</div>
					</div>
				</div>
				...
				<div class="ui fluid red shadowless segment">
				...
				<div class="field">
					<button class="ui blue button" type="submit">{{ __('Save changes') }}</button>
				</div>
			</form>
		</div>
				...
				<table class="ui basic large unstackable table">
					...
					<tbody>
						...
						<tr>
							...
							<td><button class="ui button" type="button" @click="downloadItem({{ $invoice->id }}, '#download-invoice')">{{ __('Export') }}</button></td>
							@else
							<td><button class="ui button" type="button" @click="downloadInvoice({{ $invoice->id }})">{{ __('Export') }}</button></td>
							@endif
						</tr>
						@endforeach
					</tbody>
	...
	@elseif(route_is('home.user_prepaid_credits') && config('app.prepaid_credits.enabled'))
		...
		<div class="items prepaid_credits invoices">
			<div class="header extra">
				<div class="title">{{ __('Prepaid credits') }}</div>
			</div>
