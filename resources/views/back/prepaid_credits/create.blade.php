...
@section('title', __('Create prepaid credits pack'))
...
@section('content')
<form class="ui large form" method="post" action="{{ route('prepaid_credits.store') }}">
	@csrf
	...
	<div class="one column grid" id="prepaid_credits">
		<div class="column">
			...
			<div class="field">
				...
				<input type="number" name="amount" placeholder="..." value="{{ old('amount') }}" required>
			</div>
			<div class="field">
				<label>{{ __('Discount on products') }} <sup>({{ __('Percentage') }})</sup></label>
				<input type="number" name="discount" placeholder="..." value="{{ old('discount', 0) }}">
			</div>
			<div class="field">
				<label>{{ __('Popular') }}</label>
				<div class="ui selection floating dropdown">
					<input type="hidden" name="popular" value="{{ old('popular') }}">
					<div class="text">...</div>
					<div class="menu">
			...
			<div class="field">
				...
				<textarea name="specs" class="html-editor" rows="10">{{ old('specs') }}</textarea>
			</div>
		</div>
	</div>
</form>
